...
from analyses.scale_free.fit_qc_sensitivity import run_fit_qc_sensitivity
from analyses.behavioral.fit_qc_sensitivity import (
...
def main() -> None:
    ...
    parser.add_argument("--scale-free-output", default="outputs/full/scale_free")
    parser.add_argument("--bout-ordinal-output", default="outputs/full/bouts")
    parser.add_argument(
        "--participants", default="processed/metadata/subjects.csv"
    )
    parser.add_argument(
        "--behavioral-config",
        default="config/analyses/behavioral.json",
