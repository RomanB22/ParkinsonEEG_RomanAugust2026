...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Render one specparam decomposition PNG per subject/electrode from "
            "the saved scale-free spectral intermediates."
        )
    )
    ...
    if manifest_path.exists():
        ...
        manifest["specparam_gallery_policy"] = (
            "One decomposition PNG per analyzed subject/shared-electrode pair, "
            "generated from saved fitted spectral curves without refitting specparam."
        )
    ...
    print(
        f"Specparam gallery ready: {len(index)} figures ({rendered} rendered now); "
