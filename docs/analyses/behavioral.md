...
analysis, which calculates a 1–50 Hz PSD and fits fixed and knee candidates over
4–50 Hz. BIC selects
...
Its interpretation remains model-dependent: fixed is the single 4–50 Hz slope,
...
ordinal features come from `src/analyses/bouts/`. All upstream manifests are checked
...
tables are always reused from `outputs/full/ordinal`; existing complete
...
```bash
bash src/analyses/behavioral/prepare_dimension_sensitivity.sh
```

Then run:

```bash
bash src/analyses/behavioral/run_quantitative_behavioral.sh --overwrite
```
...
```bash
bash src/analyses/behavioral/run_quantitative_behavioral.sh \
